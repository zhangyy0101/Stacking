...
from gurobi_block_allocation_framework import (
    Area,
    Vessel,
    YardAllocationData,
...
def discover_data_dir(base_dir: Path, data_dir: Optional[Path]) -> Path:
    ...
    candidates = [
        ...
        if p.is_dir() and p.name.endswith("20260508")
    ]
    if not candidates:
        raise FileNotFoundError("No data directory ending with 20260508 was found")
    if len(candidates) > 1:
...
def build_yard_allocation_data(
    *,
    theta: pd.Timestamp,                        # 当前滚动规划时刻
...
def main() -> None:
    ...
    # 开始构造完整的模型输入
    artifacts = build_yard_allocation_data(
        theta=theta,
