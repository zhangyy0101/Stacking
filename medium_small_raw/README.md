...
- 求解时直接决策资料箱细分组和预测兜底组到箱区、贝位、排号和连续 6 小贝区块的分配。
- `small_plan.csv` 输出 `row_no` 和 `row_allocation`，排级容量、排级尺寸容量和排级不混属性在列生成、主问题和 repair 中都是硬约束。
...
```text
某个资料箱细分组或预测兜底组 -> 某个箱区的某个贝位和排级分配，固定箱量 q
```
...
- 45ft 必须放箱区边贝；
- 同贝位不混尺寸、不混箱高；
- 同一排不混卸港，输入中的 `attribute_rules` 可以覆盖贝位/排不混属性；
- 当前堆场已有箱的尺寸、箱高约束会被保留；
...
- repair 结束后执行箱区内贝位重排：固定每个细属性组在每个箱区的箱量，只重新安排箱区内部贝位；排级分配只用于证明贝位方案满足容量和不混排硬约束。
...
如果中计划已经由外部给定，可以只运行小计划入口。外部中计划需要包含 `voyage_id`、`flow`、`port`、`size`、`area_no`、`planned_boxes` 这些字段；字段名也兼容 `voy_id/status/planned_qty` 等常见别名。若外部中计划包含 `bay_key` 或 `bay_no`，程序同时使用粗属性组-箱区-贝位硬配额。输出的小计划按粗属性组汇总后不会超过对应配额。

```powershell
.\.venv\Scripts\python.exe medium_small_raw\run_small_plan_from_medium.py `
...
- `--medium-small-group-area-split-penalty`：小量粗属性组使用额外箱区的惩罚，默认 `2400`。
- `--medium-small-group-fragment-penalty`：小量粗属性组没有放在最大箱区的碎片箱量惩罚，默认 `90`。
...
repair 后的箱区内贝位重排默认开启。它不改变箱区层方案、不改变大计划继承比例，也不改变粗属性组在每个箱区的合计箱量；它只在同一箱区内让细属性组和粗属性组尽量少跨贝位，并减少小尾巴贝位。若某个箱区无法在排级硬约束下完成重排，该箱区保留 repair 后的原贝位方案；只有整体贝位集中性更好时，重排方案才会被接受。`post_repair_area_relayout_max_patterns` 默认为 `1`，表示每个候选贝位只取一个排级可行模式，排号主要用于可行性校验。
