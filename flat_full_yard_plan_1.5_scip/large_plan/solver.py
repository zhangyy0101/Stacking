...
class DailyRollingYardPlanningData:
    ...
    bay_limit_areas_by_vessel: Mapping[Vessel, Sequence[Area]] = field(default_factory=dict)
    bay_limit_C20: Mapping[VA, float] = field(default_factory=dict)
    bay_limit_C20Direct: Mapping[VA, float] = field(default_factory=dict)
    bay_limit_C40: Mapping[VA, float] = field(default_factory=dict)
...
class DailyRollingYardPlanningSolution:
    ...
    objective_components_normalized: Dict[str, float] = field(default_factory=dict)
    objective_layer_values: Dict[str, float] = field(default_factory=dict)
...
def _objective_normalizers(params: Mapping[str, Any]) -> dict[str, float]:
    total_demand = sum(params["D20"].values()) + sum(params["D40"].values())
    of_demand = sum(
        params["D20"][v, "OF"] + params["D40"][v, "OF"]
        for v in params["OF_area_vessels"]
    )
    area_count = len(params["A"])
    of_area_count = max(1, len(params["OF_area_vessels"]))
    return {
        "miss": _positive_scale(total_demand),
        "required_area": _positive_scale(len(params["required_area_keys"])),
        "priority_area": _positive_scale(len(params["priority_area_keys"])),
        "import_cluster_area": _positive_scale(len(params["import_cluster_area_keys"])),
        "balance": _positive_scale(of_demand),
        "of_area": _positive_scale(area_count * of_area_count),
        "operation": _positive_scale(total_demand),
        "share": _positive_scale(area_count),
        "berth_conflict": _positive_scale(len(params["berth_conflict_keys"])),
        "distance": _positive_scale(total_demand),
        "adjustment": _positive_scale(total_demand),
    }


def _build_hierarchical_objectives(
    components: Mapping[str, Any],
    params: Mapping[str, Any],
) -> tuple[list[dict[str, Any]], dict[str, Any], dict[str, float], dict[str, list[str]]]:
    normalizers = _objective_normalizers(params)
    normalized = {
        name: components[name] / normalizers[name]
        for name in normalizers
        if name in components
    }
    layers = [
        {
            "name": "L1_demand_satisfaction",
            "expr": normalized["miss"],
            "terms": ["miss"],
        },
        {
            "name": "L2_manual_priority_cluster",
            "expr": (
                1.0 * normalized["required_area"]
                + 0.1 * normalized["priority_area"]
                + 0.01 * normalized["import_cluster_area"]
            ),
            "terms": ["required_area", "priority_area", "import_cluster_area"],
        },
        {
            "name": "L3_of_balance_and_area_count",
            "expr": 1.0 * normalized["balance"] + 0.1 * normalized["of_area"],
            "terms": ["balance", "of_area"],
        },
        {
            "name": "L4_operational_quality",
            "expr": (
                1.0 * normalized["operation"]
                + 0.1 * normalized["share"]
                + 0.01 * normalized["berth_conflict"]
                + 0.001 * normalized["distance"]
                + 0.0001 * normalized["adjustment"]
            ),
            "terms": ["operation", "share", "berth_conflict", "distance", "adjustment"],
        },
    ]
    layer_terms = {layer["name"]: list(layer["terms"]) for layer in layers}
    return layers, normalized, normalizers, layer_terms


def _hierarchical_bound(value: float) -> float:
    numeric = float(value)
    return numeric + 1e-6 + 1e-6 * max(1.0, abs(numeric))
...
def build_daily_rolling_yard_model(
    ...
    *,
    ...
    # 约束 8.1：用户限定中小计划贝位对应的大计划新增容量硬约束。
    # 仅限制 X-S，即未在当前快照中的新增计划箱量。
    for v, a in params["bay_limit_keys"]:
        model.addConstr(
            gp.quicksum(X20[v, f, a] - params["S20"][v, f, a] for f in F)
            <= params["bay_limit_C20Direct"][v, a],
            name=f"user_bay_capacity20_direct[{v},{a}]",
        )
        model.addConstr(
            gp.quicksum(
                (X20[v, f, a] - params["S20"][v, f, a])
            ...
            <= params["bay_limit_C20"][v, a],
            name=f"user_bay_capacity20[{v},{a}]",
        )
        model.addConstr(
            gp.quicksum(X40[v, f, a] - params["S40"][v, f, a] for f in F)
            <= params["bay_limit_C40"][v, a],
            name=f"user_bay_capacity40[{v},{a}]",
        )
    ...
    # m_of 表示该航次 OF 箱在任一箱区的最大最终计划量。最小化 m_of 可以避免
    ...
    if "OF" in F:
        for v in OF_area_vessels:
            for a in A:
                model.addConstr(
                    X20[v, "OF", a] + X40[v, "OF", a] <= m_of[v],
                    name=f"of_max_total[{v},{a}]",
                )
    ...
    # Z_bal：各出口航次 OF 总箱量的最大单箱区最终计划量。
    Z_bal = gp.quicksum(m_of[v] for v in OF_area_vessels)
    ...
    layers, normalized_components, normalizers, layer_terms = _build_hierarchical_objectives(
        variables["objective_components"],
        params,
    )
    variables["objective_layers"] = layers
    variables["objective_components_normalized"] = normalized_components
    variables["objective_normalizers"] = normalizers
    variables["objective_layer_terms"] = layer_terms
    model.setObjective(layers[0]["expr"], GRB.MINIMIZE)
    return model, variables, params


def _optimize_gurobi_hierarchical(model: Any, objective_layers: Sequence[Mapping[str, Any]]) -> None:
    if not objective_layers:
        model.optimize()
        return
    for index, layer in enumerate(objective_layers):
        model.setObjective(layer["expr"], GRB.MINIMIZE)
        model.optimize()
        if model.SolCount <= 0 or index == len(objective_layers) - 1:
            return
        value = float(layer["expr"].getValue())
        model.addConstr(
            layer["expr"] <= _hierarchical_bound(value),
            name=f"hierarchical_fix_{index + 1}_{layer['name']}",
        )
...
def solve_daily_rolling_yard_plan(
    ...
    *,
    ...
    _optimize_gurobi_hierarchical(model, variables.get("objective_layers", []))
    ...
    if model.SolCount <= 0:
        return DailyRollingYardPlanningSolution(
            ...
            required_area_unmet={},
            objective_components={},
            objective_components_normalized={},
            objective_layer_values={},
    ...
    normalized_components = variables.get("objective_components_normalized", {})
    objective_layers = variables.get("objective_layers", [])
    ...
    return DailyRollingYardPlanningSolution(
        ...
        m_of=_extract_float_tupledict(variables["m_of"], (OF_area_vessels,)),
        ...
        objective_components_normalized={name: expr.getValue() for name, expr in normalized_components.items()},
        objective_layer_values={layer["name"]: layer["expr"].getValue() for layer in objective_layers},
        model=model if keep_model else None,
    )
...
def _prepare_params(
    ...
    bay_limit_areas_by_vessel = _normalize_sparse_area_controls(
        getattr(data, "bay_limit_areas_by_vessel", {}),
        V,
        A,
    )
        ...
        for f in F:
            for a in A:
                if a not in allowed_areas:
                    E20[v, f, a] = 0
                    E40[v, f, a] = 0
    for v, limited_areas in bay_limit_areas_by_vessel.items():
        for f in F:
            for a in A:
                if a not in limited_areas:
                    E20[v, f, a] = 0
                    E40[v, f, a] = 0
    ...
    bay_limit_keys = [
        (v, a)
        for v, limited_areas in bay_limit_areas_by_vessel.items()
        for a in sorted(limited_areas)
    ]
    bay_limit_C20 = {(v, a): _num(data.bay_limit_C20, (v, a), 0.0) for v, a in bay_limit_keys}
    bay_limit_C20Direct = {
        (v, a): _num(data.bay_limit_C20Direct, (v, a), bay_limit_C20[v, a])
        for v, a in bay_limit_keys
    }
    bay_limit_C40 = {(v, a): _num(data.bay_limit_C40, (v, a), 0.0) for v, a in bay_limit_keys}
    ...
    _validate_nonnegative("bay_limit_C20", bay_limit_C20)
    _validate_nonnegative("bay_limit_C20Direct", bay_limit_C20Direct)
    _validate_nonnegative("bay_limit_C40", bay_limit_C40)
    ...
    return {
        ...
        "bay_limit_keys": bay_limit_keys,
        "bay_limit_C20": bay_limit_C20,
        "bay_limit_C20Direct": bay_limit_C20Direct,
...
def _normalize_sparse_area_controls(
    raw: Mapping[Vessel, Sequence[Area]],
    vessels: Sequence[Vessel],
    areas: Sequence[Area],
    ...
    for raw_vessel, raw_areas in (raw or {}).items():
        vessel = str(raw_vessel)
        if vessel not in vessel_set:
            continue
        normalized = {str(area) for area in (raw_areas or ()) if str(area) in area_set}
...
def build_daily_rolling_yard_model_scip(
    ...
    *,
    ...
    m_of = {v: model.addVar(vtype="C", lb=0.0, name=f"m_of[{v}]") for v in OF_area_vessels}
    ...
    if "OF" in F:
        for v in OF_area_vessels:
            for a in A:
                model.addCons(X20[v, "OF", a] + X40[v, "OF", a] <= m_of[v])
    ...
    Z_bal = quicksum(m_of[v] for v in OF_area_vessels)
    ...
    variables["objective_normalizers"] = normalizers
    variables["objective_layer_terms"] = layer_terms
    model.setObjective(layers[0]["expr"], "minimize")
    return model, variables, params


def _optimize_scip_hierarchical(model: Any, objective_layers: Sequence[Mapping[str, Any]]) -> None:
    if not objective_layers:
        model.optimize()
        return
    for index, layer in enumerate(objective_layers):
        model.setObjective(layer["expr"], "minimize")
        model.optimize()
        if model.getNSols() <= 0 or index == len(objective_layers) - 1:
            return
        value = _safe_scip_expr_value(model, layer["expr"])
        if hasattr(model, "freeTransform"):
            model.freeTransform()
        model.addCons(
            layer["expr"] <= _hierarchical_bound(value),
            name=f"hierarchical_fix_{index + 1}_{layer['name']}",
...
def solve_daily_rolling_yard_plan(
    ...
    *,
    ...
    _optimize_scip_hierarchical(model, variables.get("objective_layers", []))
    ...
    return DailyRollingYardPlanningSolution(
        ...
        m_of=_extract_scip_float_vars(model, variables["m_of"], (OF_area_vessels,)),
        ...
        objective_components_normalized={
            name: _safe_scip_expr_value(model, expr) for name, expr in normalized_components.items()
        },
        objective_layer_values={
            layer["name"]: _safe_scip_expr_value(model, layer["expr"]) for layer in objective_layers
        },
        model=model if keep_model else None,
    )
