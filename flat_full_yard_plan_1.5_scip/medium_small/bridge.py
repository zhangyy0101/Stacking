...
def add_column_generation_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--twenty-max-consecutive-bays",
        type=int,
        default=defaults.twenty_max_consecutive_bays,
        help="Preferred maximum number of consecutive small bays used by 20ft boxes; use 0 to disable.",
    )
    parser.add_argument(
        "--twenty-consecutive-violation-penalty",
        type=float,
        default=defaults.twenty_consecutive_violation_penalty,
        help="Secondary soft cost per sliding-window violation after unplaced-box priorities.",
    )
...
def make_config(args: argparse.Namespace) -> ColumnGenerationConfig:
    ...
    return ColumnGenerationConfig(
        ...
        twenty_max_consecutive_bays=getattr(
            args,
            "twenty_max_consecutive_bays",
            defaults.twenty_max_consecutive_bays,
        ),
        twenty_consecutive_violation_penalty=getattr(
            args,
            "twenty_consecutive_violation_penalty",
            defaults.twenty_consecutive_violation_penalty,
        ),
