...
def read_export_voyages(data_dir: str | Path) -> set[str]:
    """Read voyage direction without requiring a complete export schedule."""
    data_path = Path(data_dir)
    exports: set[str] = set()
    if has_input_json(data_path):
        take_over = input_value(data_path, "take_over_vessel", {})
        if isinstance(take_over, dict):
            exports.update(_voyage(voyage_id) for voyage_id in take_over.get("E", []) if _voyage(voyage_id))
        containers = input_value(data_path, "vessel_containers", {})
        if isinstance(containers, dict):
            exports.update(
                _voyage(voyage_id)
                for voyage_id, content in containers.items()
                if isinstance(content, dict) and _norm(content.get("type")).upper() == "E" and _voyage(voyage_id)
            )
        frame = input_dataframe(data_path, "vessel_berth_info")
    else:
        path = data_path / "vessel_berth_info_new.csv"
        if not path.exists():
            path = data_path / "vessel_berth_info.csv"
        frame = _read_csv_compat(path) if path.exists() else pd.DataFrame()
    if not frame.empty:
        exports.update(
            _voyage(row.get("VOY_ID"))
            for row in frame.to_dict("records")
            if _norm(row.get("VOY_IEFG")).upper() == "E" and _voyage(row.get("VOY_ID"))
        )
    return exports
...
def load_port_demand_groups(
    ...
    data_path = Path(data_dir)
    export_voyages = read_export_voyages(data_path)
...
def enforce_medium_groups_cover_small_groups(
    ...
    export_voyages = set(export_voyages or set())
    for index, group in enumerate(updated):
        key = _configured_group_coarse_key(group, attribute_rules, export_voyages)
...
def _configured_group_coarse_attrs(
    group: BoxGroup | SmallBoxGroup,
    attribute_rules: AttributeRules | None,
    export_voyages: set[str],
) -> tuple[str, ...]:
    if str(getattr(group, "voyage_id", "")) not in export_voyages:
...
def _configured_group_coarse_key(
    group: BoxGroup | SmallBoxGroup,
    attribute_rules: AttributeRules | None,
    export_voyages: set[str],
) -> tuple[str, ...]:
    attrs = _configured_group_coarse_attrs(group, attribute_rules, export_voyages)
...
def _populate_existing_bay_attributes(
    ...
    for bay_key, values in occupied.groupby("_bay_key", sort=False):
        ...
        for row_no, row_values in values.groupby("_row", sort=False):
            ...
            export_voyages = {
                str(item)
                for item in row_values["_voyage"].dropna().unique()
                if str(item)
            }
            if export_voyages:
                bay.existing_attrs_by_row.setdefault(row_key, {}).setdefault(
                    EXPORT_VOYAGE_ROW_NO_MIX_ATTR,
                    set(),
                ).update(export_voyages)
...
def build_problem(
    ...
    export_voyages = read_export_voyages(data_dir) & set(target_voyages)
    groups, medium_doc_floor_added_by_coarse_group, medium_doc_floor_shifted_by_coarse_group = (
        enforce_medium_groups_cover_small_groups(groups, small_groups, attribute_rules, export_voyages)
    )
