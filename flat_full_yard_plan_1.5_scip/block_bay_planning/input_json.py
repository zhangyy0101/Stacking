...
def input_voyage_value(data_dir: str | Path, key: str, voyage_id: object, default: Any = None) -> Any:
    value = input_value(data_dir, key, default)
    if not isinstance(value, dict):
        return value
    voyage = _voyage(voyage_id)
    return value.get(voyage, value.get(str(voyage_id), default))
