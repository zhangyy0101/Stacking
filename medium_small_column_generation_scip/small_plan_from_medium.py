...
from medium_small_column_generation_scip.block_bay_planning.models import BigPlanRow, ProblemData
from medium_small_column_generation_scip.column_generation_planner import ColumnGenerationConfig
...
def read_external_medium_plan(path: str | Path) -> ExternalMediumPlan:
    ...
    return ExternalMediumPlan(
        rows=sorted(rows, key=lambda row: (row["voyage_id"], row["flow"], row["area_no"], row["port"], row["size"])),
