...
\begin{enumerate}
  ...
  \item 构建中小计划 \code{ProblemData}，包括实际需求、贝位容量、箱区功能、用户箱区和贝位控制、大计划配额、堆场现状协调信息等。
...
中计划 & 实际需求、大计划箱区配额、贝位容量、箱区功能、用户粗属性规则、用户箱区规则、堆场现状协调信息 & 输出粗属性组到箱区和贝位的分配结果；小计划继承其箱区和贝位边界 \\
...
该 JSON 内包含航次、在场箱、资料箱、预测箱、箱区功能、贝位容量、用户规则等结构化数据。全流程直接基于该对象构建大计划和中小计划输入。
...
\begin{enumerate}
  \item 计算实际待分配需求时，扣除目标航次已经在场的箱。
  \item 构建堆场现状协调信息，用于在位置选择时参考目标航次已有在场箱的分布。
\end{enumerate}
...
中计划组的生成逻辑为：先计算航次、流向、尺寸、港口层面的实际需求，再按上述粗属性口径拆成若干粗属性组。如果资料箱中存在对应记录，则按资料箱中的真实属性比例分摊需求；如果没有资料箱记录，则使用默认或兜底属性值。

小计划组的生成逻辑为：以资料箱为基础，先扣除已经在场的箱，再按上述细属性口径形成细属性组。\param{bay_rules}、\param{row_rules} 和重量档字段会随小计划组携带，用于贝位、排和重量相关约束，但不额外定义粗属性组。堆场现状协调信息也使用同一粗属性口径，不再使用代码内部固定的粗属性组合。

\subsection{实际需求计算}

中小计划需求由 \code{calculate_medium_demands()} 计算。需求口径为资料箱、预测箱和在场箱扣减后的实际需求，大计划 \code{new_qty} 不作为需求上界。
...
在场箱扣减会按箱号或箱 ID 去重，并排除同时关联进口航次和出口航次的在场中转箱。该排除口径也用于堆场现状协调信息，避免此类中转箱同时影响进口和出口计划。
...
因此，即使实际需求与大计划配额不一致，中小计划仍按实际需求尝试落贝，并通过软惩罚尽量继承大计划。
...
堆场现状 & 候选贝若已有同内部粗 key 的在场箱会得到奖励，邻近已有同 key 贝位也会得到较小奖励；与其他 key 冲突或邻近冲突会增加成本。 \\
...
目标类别 & 组合选择含义 \\
\midrule
减少未放置箱 & 未放置箱有高惩罚，主问题优先覆盖实际需求。 \\
大计划偏离 & 按航次、流向、箱区、尺寸统计实际使用量，与大计划配额目标的差值越大惩罚越高。 \\
中计划集中度 & 对需要集中的内部粗 key，跨多个箱区会产生惩罚，最大箱区份额会产生奖励；对较大的组，会控制打开箱区数量、过小箱区份额和箱区间分布失衡。 \\
小计划拆分 & 同一细属性组跨箱区、跨块、跨贝都会增加惩罚；同一内部粗 key 在同箱区内跨块、跨贝也会增加惩罚。 \\
属性兼容性 & 主问题会继续保证同一贝位或同一排内不会选择互相冲突的属性取值。 \\
\bottomrule
\end{longtable}

因此，中小计划最终输出的位置，是在满足功能、容量、用户范围和属性混放等硬约束的基础上，对大计划继承、实际需求覆盖、粗细属性集中、堆场现状、作业距离和特殊箱区规则加权后的整体最优或近似最优组合。
...
\pathcode{medium_small_plan/medium_demand_by_port.csv} & 中计划实际需求明细 \\
...
参数 & 默认值 & 说明 \\
\midrule
\param{demand_mode} & \code{original} & 需求模式，可选 \code{original}、\code{medium}、\code{medium-with-doc-floor}、\code{doc-only} \\
...
参数 & 默认值 & 说明 \\
\midrule
\param{unplaced_penalty} & 100000.0 & 未放置箱惩罚 \\
