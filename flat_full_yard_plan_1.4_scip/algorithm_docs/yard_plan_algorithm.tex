...
\begin{itemize}
  ...
  \item 矫正大计划生成：\code{medium_small/corrected_large_plan.py}
...
\begin{enumerate}
  ...
  \item 根据原大计划和中计划实际新增箱落位生成矫正大计划，使大计划箱区结果覆盖中小计划实际分配。
\end{enumerate}
...
矫正大计划 & 原大计划 \code{allocation.csv}、中计划实际新增箱落位 & 输出覆盖中计划新增箱分配的箱区级大计划视图；保留原在场箱快照，按新增箱口径矫正 \code{new_qty} \\
\bottomrule
\end{longtable}
...
中小计划的贝位容量、行容量、尺寸混放和大贝 partner 占用按当前物理在场状态处理，不按计划离港时间释放快照中的箱位。

待分配新箱与在场箱的兼容性按两层口径处理：
\begin{itemize}
  \item 尺寸约束跨航次生效。只要贝位或排内已有在场箱，新箱必须与该位置的尺寸状态兼容；20、40、45 尺在中小计划中保持独立尺寸口径。
  \item \param{bay_rules} 和 \param{row_rules} 中的非尺寸属性按航次作用域生效。若在场箱属于同一航次，则这些用户配置属性会约束新箱；若在场箱不属于同一航次，则不以该在场箱的非尺寸属性限制新箱。
\end{itemize}

快照中已经存在的历史混贝、混排状态不作为中小计划的修复对象。算法只保证新增落位不继续制造与当前新箱组不兼容的尺寸、贝位属性和排属性冲突。
...
常见字段包括 \code{voy_id}、\code{flow}、\code{area_no}、\code{size}、\code{planned_qty}、\code{snapshot_qty}、\code{new_qty}。

\code{snapshot_qty} 表示当前快照中已经在该箱区的同航次、同流向、同尺寸口径箱量；\code{new_qty} 表示仍需新增安排的箱量；\code{planned_qty} 表示两者合计后的箱区级总计划量。
...
column 生成时先做功能、容量、用户范围、属性兼容等硬过滤；通过过滤后，再计算大计划继承、堆场现状协调、作业位置、20 尺空间利用等成本项。具体口径在下面的位置选择章节中说明。
...
\begin{itemize}
  ...
  \item 当前在场箱和已选候选列形成的贝位、排属性状态必须满足尺寸约束、\param{bay_rules} 和 \param{row_rules}。
  \item 单独小计划入口中，如果配置了外部中计划配额，repair 的部分阶段会把中计划箱区或贝位配额作为阶段性范围约束。
\end{itemize}

用户指定箱区不会强行覆盖箱区功能。也就是说，可用箱区是用户范围、关闭/禁用规则、箱区功能、容量和属性规则共同过滤后的交集。

尺寸约束是固定硬约束，不依赖用户是否显式把尺寸写入 \param{bay_rules}。规划器会把尺寸属性并入贝位不混规则，且尺寸属性不区分航次；因此，新箱之间、新箱与在场箱之间都不能在同一约束作用范围内形成 20/40/45 尺寸冲突。

40/45 尺落位还会检查大贝 partner。对于由两个小贝组成的大贝，两个小贝必须同时具备可用容量和兼容的尺寸状态；如果某个小贝已有 20 尺在场箱，则该小贝不参与大贝组合。如果快照中已有 40/45 尺箱占用一组小贝，则该组以及与该组成员重叠的其他大贝组合都会被阻断。例如 07+09 已有大箱、11+13 已有大箱时，09+11 不能再作为新大贝使用。
...
20 尺空间利用 & 20 尺箱使用孤立空小贝或不降低剩余大贝组合能力的位置时成本较低；打断连续可拼大贝的小贝段时会增加成本。已有在场箱的非段内小贝不额外获得孤立空小贝奖励。 \\
...
20 尺空间利用 & 对 20 尺方案统计连续可拼大贝小贝段的组合能力损失；repair 过程中也会根据已选 20 尺落位动态更新该成本，尽量减少新开小贝对 40/45 尺能力的破坏。 \\
属性兼容性 & 主问题会继续保证同一贝位或同一排内不会选择互相冲突的属性取值。 \\
\bottomrule
\end{longtable}

因此，中小计划最终输出的位置，是在满足功能、容量、用户范围和属性混放等硬约束的基础上，对大计划继承、计划需求覆盖、资料箱优先、粗细属性集中、堆场现状、20 尺空间利用、作业距离和特殊箱区规则加权后的整体最优或近似最优组合。

\subsection{主问题}

列生成主问题在上述候选列成本和组合层惩罚基础上选择 column，覆盖所有细属性组需求。
...
\begin{itemize}
  ...
  \item 改善小计划集中度、贝位和块拆分、堆场现状协调、20 尺空间利用等指标。
...
矫正大计划是中小计划完成后的箱区级结果视图，用于把中小计划实际落位结果回写到大计划口径。该过程不重新求解大计划，只对原大计划箱区行做汇总、覆盖和重分布。

\subsection{输入和适用入口}

矫正大计划使用两个输入：
\begin{itemize}
  \item 原大计划行：全流程中使用内存中的 \code{allocation.csv} 行；单独中小计划中读取 \param{--big-plan} 指向的大计划 CSV。
  \item 中计划行：中小计划输出的 \code{medium_plan.csv}，其中 \code{planned_boxes} 表示新增箱落位量，不包含已经在场的 \code{snapshot_qty}。
\end{itemize}

全流程入口和单独中小计划入口均输出矫正大计划。对应脚本为
\pathcode{run_flat_full_yard_plan.py} 和
\pathcode{run_medium_small_column_generation.py}。
单独大计划入口只输出原大计划；单独小计划入口只根据外部中计划生成小计划，不生成矫正大计划。

\subsection{基本口径}

矫正大计划按 \((\text{航次}, \text{流向}, \text{尺寸})\) 分组，45 尺按大计划口径并入 40 尺。对每个箱区 \(a\)，定义：

\[
\begin{aligned}
S_a &= \text{原大计划中的 snapshot\_qty},\\
N^{old}_a &= \text{原大计划中的 new\_qty},\\
M_a &= \text{中计划实际分配到该箱区的新增箱量}.
\end{aligned}
\]

其中 \(M_a\) 是新增箱量，不包含在场箱。矫正后保持：
\[
S^{corr}_a = S_a
\]
并以中计划落位作为新增量下界：
...
最终输出字段满足：
\[
\text{planned\_qty}^{corr}_a
= \text{snapshot\_qty}^{corr}_a
+ \text{new\_qty}^{corr}_a.
\]

\subsection{新增量目标}

对每个 \((v,f,s)\) 分组，矫正后的新增总量目标为：
\[
N^{target}
= \max\left(
\sum_a N^{old}_a,
...
该口径体现两点：
\begin{itemize}
  \item 原大计划已经给出的新增配额会尽量保留。
  \item 当原大计划存在未满足需求，导致原 \code{new_qty} 少于中计划实际落位时，矫正大计划允许扩展 \code{new_qty}，以覆盖中小计划结果。
\end{itemize}
...
箱区级矫正先取：
\[
N^{corr}_a = \max(N^{old}_a,\ M_a).
\]

如果 \(\sum_a N^{corr}_a > N^{target}\)，则只从超过中计划下界的部分中扣减，即扣减量不使任何箱区低于 \(M_a\)。扣减按可扣减量使用最大余数法分配。

如果 \(\sum_a N^{corr}_a < N^{target}\)，则把剩余新增量分配回箱区。优先按原大计划 \code{new_qty} 权重分配；没有原新增量时，按中计划落位量分配；仍无权重时，按在场箱快照权重兜底。

因此，矫正大计划既覆盖中计划实际新增落位，又尽量保留原大计划新增配额总量。原大计划因未满足需求而没有放下的新增箱，可以通过中小计划实际落位被补入矫正大计划。

\subsection{诊断口径}

矫正诊断文件记录：
\begin{itemize}
  \item \code{coverage_violation_count}：矫正后 \code{new_qty} 未覆盖中计划实际落位的行数。
  \item \code{feasible}：当覆盖缺口为 0 时为 \code{true}。
  \item \code{original_snapshot_total} 和 \code{corrected_snapshot_total}：用于确认在场箱快照保持不变。
  \item \code{original_new_total} 和 \code{corrected_new_total}：用于比较新增量矫正前后差异。
  \item \code{new_total_expansion_boxes}：因中计划实际落位超过原大计划 \code{new_qty} 而补入的新增箱总量。
  \item \code{expanded_groups}：发生新增量扩展的 \((\text{航次}, \text{流向}, \text{尺寸})\) 分组。
\end{itemize}
...
\pathcode{medium_small_plan/corrected_large_plan.csv} & 矫正大计划结果；保留在场箱快照，覆盖中计划新增箱落位 \\
\pathcode{medium_small_plan/corrected_large_plan_diagnostics.json} & 矫正大计划诊断信息 \\
...
\begin{Verbatim}[fontsize=\small,breaklines=true,breakanywhere=true]
medium_small_column_generation_outputs/<run_name>/
\end{Verbatim}

该入口同样输出 \pathcode{corrected_large_plan.csv} 和
\pathcode{corrected_large_plan_diagnostics.json}。其原大计划输入来自命令行参数 \param{--big-plan}。
...
\param{twenty_isolated_bay_reward} & 80.0 & 20 尺箱使用孤立空小贝或静态不损失大贝能力位置的奖励 \\
\param{twenty_large_segment_loss_penalty} & 220.0 & 最终方案造成连续小贝段大贝组合能力损失时的组合层惩罚 \\
\param{twenty_large_segment_fresh_loss_penalty} & 240.0 & repair 动态选择中，新使用某个小贝会减少剩余大贝组合能力时的惩罚 \\
\param{twenty_large_segment_used_zero_loss_reward} & 160.0 & repair 动态选择中，同一连续段已有 20 尺落位后，继续使用不增加大贝损失位置的奖励 \\
