...
def project_root_from(base_dir: Path) -> Path:
    ...
    return base_dir.parent if base_dir.name == "large_plan" else base_dir
...
def resolve_output_path(path: Path, base_dir: Path) -> Path:
    ...
    if path.is_absolute():
        return path.resolve()
    return (project_root_from(base_dir) / path).resolve()
...
def discover_data_dir(base_dir: Path, data_dir: Optional[Path]) -> Path:
    if data_dir is not None:
        ...
        return resolved
    return project_root_from(base_dir)


def discover_area_function_file(data_dir: Path) -> Path:
    exact = data_dir / "箱区功能.xlsx"
    if exact.exists():
        return exact
    candidates = [
        p
...
def generate_yard_visualization(
    *,
    ...
    resolved_allocation = resolve_input_path(allocation_path, base_dir)
    ...
    return {
        ...
        "allocation_path": str(resolved_allocation),
        "output_dir": str(resolved_output_dir),
        "base_image_copy": str(blank_path),
        "annotated_image": str(annotated_path),
        "visualization_check": str(check_path),
        "unmapped_areas_path": str(unmapped_path) if unmapped_path else None,
