...
from block_bay_planning.models import BigPlanRow, ProblemData
from medium_small.column_generation_planner import ColumnGenerationConfig
...
def read_external_medium_plan(path: str | Path) -> ExternalMediumPlan:
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as fh:
        ...
        return external_medium_plan_from_records(list(reader), list(reader.fieldnames), source=str(path))


def external_medium_plan_from_records(
    records: list[dict],
    fieldnames: list[str] | None = None,
    source: str = "medium plan dataframe",
) -> ExternalMediumPlan:
    if fieldnames is None:
        fieldnames = sorted({key for row in records for key in row})
    if not fieldnames:
        raise ValueError(f"medium plan has no header: {source}")
    ...
    for raw in records:
        ...
        rows.append(row)
    if not coarse_quota:
        raise ValueError(f"medium plan has no positive planned boxes: {source}")
    big_plan_rows = [
        BigPlanRow(
            voyage_id=voyage_id,
            flow=flow,
...
def _optional_bay_key(row: dict, field_map: dict[str, str], area_no: str) -> str:
    ...
    if bay_key:
        return _canonical_bay_key(bay_key, area_no)
    bay_no = _get_optional(row, field_map, "bay_no")
    if bay_no:
        return _canonical_bay_key(bay_no, area_no)
    return ""


def _canonical_bay_key(value: object, area_no: str) -> str:
    ...
    if "-" in text:
        left, right = text.split("-", 1)
        if left.strip() == area_no:
            return f"{area_no}|{right.strip()}"
    return f"{area_no}|{text}"
