...
class AreaPlanSolverMixin:
    ...
    def _area_free_capacity(
        ...
        quota_free = self.problem.area_size_quota.get(quota_key, 0) - quota_load[quota_key]
    ...
    def _medium_feasible_after(
        ...
        return quota_load[quota_key] + qty <= self.problem.area_size_quota.get(quota_key, 0)
    ...
    def _check_medium_hard_constraints(self, assignment: MediumAssignment) -> bool:
        ...
        for key, load in quota_load.items():
            if load > self.problem.area_size_quota.get(key, 0):
                return False
        return True
